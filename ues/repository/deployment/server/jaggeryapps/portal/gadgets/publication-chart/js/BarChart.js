function BarChart(area, data, options) {
    ...
    this.draw = function() {
		...
		$("#bar-tooltip" + parseInt(barNum)).hide();
        ...
        var xScale = d3.scale.ordinal()
                .domain(dataArray.map(function(d, i) {
                    return d.name;
                }))
        ...
        barChartGroup.selectAll("rect")
				...
				.on("click", function(d, i){
					if(d.type == "org"){
						window.open("https://localhost:9443/innovation-dashboard/organization.jag?oid=" + d.id);
					} else if(d.type == "per"){
						window.open("https://localhost:9443/innovation-dashboard/person.jag?pid=" + d.id);
					}
